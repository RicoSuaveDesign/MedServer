...
(tag_id int unsigned not null auto_increment, med_name varchar(100) not null, medFreqPerTime int not null, medFreqInterval bigint unsigned, dosage double not null, unit varchar(30), expiration date, dosesLeft int, taken tinyint, user_id int unsigned not null, PRIMARY KEY(tag_id), FOREIGN KEY (user_id) REFERENCES USERS(user_id));
